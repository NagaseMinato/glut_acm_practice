...
template = """# GLUT ACM Practice Summary
...
$ make -j8
$ ../bin/{programName}  # try to run a built program
$ make clean
```
...
def formatTemplate(xmlNode):
    global template
    return template.format(
        topic=lab.find('topic').text,
        summary=lab.find('summary').text,
        demand=lab.find('demand').text,
        interface=lab.find('interface').text,
        detailDesign=lab.find('detailDesign').text,
        pseudocode=lab.find('pseudocode').text,
        programName=lab.find('programName').text)


def write2Markdown(filename, str):
    fh = open("./public/"+filename, 'w',encoding='utf8')
    fh.write(str)
    fh.close()


root = ElementTree.parse(r'autogen.xml')

i = 1
for lab in root.getiterator('lab'):
    content = formatTemplate(lab)
    write2Markdown("lab"+str(i)+".md",content)
    i += 1
